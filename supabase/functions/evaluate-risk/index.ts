...
function validateRequest(body: EvaluateRiskRequest) {
  const projectedIncome = requiredNumber(
    body.projected_income,
    "projected_income",
  );
  const projectedExpenses = requiredNumber(
    body.projected_expenses,
    "projected_expenses",
  );
  const projectedBalance = requiredNumber(
    body.projected_balance,
    "projected_balance",
  );

  if (
    !isString(body.projection_date) || body.projection_date.trim().length === 0
  ) {
...
function evaluateRules(input: ReturnType<typeof validateRequest>): RuleResult {
  ...
  if (expenseCoverageRatio < 0.25) {
    reasons.push(
      "Projected balance covers less than 25% of projected expenses.",
    );
  }
  ...
  if (
    expenseCoverageRatio < 0.25 ||
    (expenseCoverageRatio < 0.5 && netCashFlow < 0)
  ) {
    return {
      risk_level: "high",
      should_alert: true,
...
async function generateRiskMessage(
  ...
  const text = await callClaude({
    ...
    prompt:
      `Write one short, natural-language message for this cash-flow risk result.
